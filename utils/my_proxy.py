...
class MyProxy:
    def __init__(self, proxy_dir: str = "proxies"):
        ...
        self.proxies = []  
        self.current_index = 0  # Initialize current index for sequential switching
        self.load_proxies()
    ...
    def next_proxy(self) -> str:
        ...
        self.current_index = (self.current_index + 1) % len(self.proxies)  # Move to the next proxy
        return proxy

    @property
    def proxy_server(self) -> str:
        return self.next_proxy()  # Use the new sequential method
