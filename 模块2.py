...
def get_files_in_directory(directory, start_file):
    ...
    files.sort()  # Assuming the filenames are in a sortable date format

    # Find the index of the start_file
    start_index = files.index(start_file)

    # Return the list of files from start_file onwards
...
def insert_empty_columns(df):
    ...
    for i, col in enumerate(cols):
        new_cols.append(col)
        if (i + 1) % 3 == 0:  # Every three columns (one date, one point, and one name), add an empty column
            new_cols.append('')
...
def process_files(directory, start_file, metrics):
    ...
    for file in files:
        ...
        if not df.empty:
            for metric in metrics:
                # Handle special cases for 'point' and 'rank'
    ...
    def prepare_data(records):
        ...
        for rank in range(1, 21):
            ...
            # Fill lists to ensure equal length
    ...
    # Write to Excel with multiple sheets and adjust column widths
