...
class SongDataFetcher:
    ...
    async def fetch_song_stat(self, index: int) -> Optional[VideoInfo]:
...
async def main():
    ...
    scraper = BilibiliScraper(mode="main")
    songs = scraper.process_old_songs(model="main")
    scraper.save_to_excel(songs)
