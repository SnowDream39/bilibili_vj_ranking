...
# 设置起始日期
today = datetime(2024, 7, 2)  # 从2024年7月2日开始处理
    ...
    if file_name.endswith(".xlsx"):
        ...
        if current_date > datetime(2024, 7, 2):
            ...
            if 'name' in previous_df.columns and 'rank' in previous_df.columns and 'point' in previous_df.columns:
                # 选择 point 最高的那一行
                last_data = previous_df.loc[previous_df.groupby('name')['point'].idxmax()][['name', 'rank', 'point']].set_index('name')

                # 填充 rank_before 和 point_before
                merged_df['rank_before'] = merged_df['name'].map(last_data['rank']).fillna('-')
                merged_df['point_before'] = merged_df['name'].map(last_data['point']).fillna('-')
